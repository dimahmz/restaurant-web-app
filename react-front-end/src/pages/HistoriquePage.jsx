...
import data from "../PosComponents/testdata.json";
...
const HistoriquePage = () => {
    ...
    const currentCustomers = data.customer.slice(
        indexOfFirstCustomer,
        indexOfLastCustomer
    );

    const pageCount = Math.ceil(data.customer.length / customersPerPage);
    const [searchQuery, setSearchQuery] = useState("");

    const [selectedBranch, setSelectedBranch] = useState(""); // State to hold the selected branch

    const handleBranchFilterChange = (selectedValue) => {
        setSelectedBranch(selectedValue);
        setSearchQuery(""); // Clear search query when a branch is selected
    };

    const filteredCustomers = data.customer.filter((customer) => {
        // Apply branch filter
        const branchFilter =
            selectedBranch === "" || customer.branch === selectedBranch;

        // Apply search filter
        const searchFilter = Object.values(customer).some((value) =>
